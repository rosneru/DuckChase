...
GameViewGfxLib::GameViewGfxLib(long width,
                               long height,
                               short depth)
  : GameViewBase(width, height, depth, 1L << depth),
    m_pOldView(NULL),
    m_View(),
    m_pViewPort(NULL),
    m_RastPort(),
...
bool GameViewGfxLib::Open(GameColors& colors)
{
  ...
  for(int i = 0; i < 2; i++)
  {
    ...
    // if they were used
    for (int iPlane = 0; iPlane < m_Depth; iPlane++)
    {
      m_pBitMapArray[i]->Planes[iPlane] = NULL;
    }

    // Allocate memory for the BitPlanes
    for (int iPlane = 0; iPlane < m_Depth; iPlane++)
    {
      m_pBitMapArray[i]->Planes[iPlane] = (PLANEPTR)
        AllocRaster(m_Width, m_Height);

      if (m_pBitMapArray[i]->Planes[iPlane] == NULL)
      {
      ...
      BltClear(m_pBitMapArray[i]->Planes[iPlane],
  ...
  m_pViewPort->DWidth = m_Width;
  m_pViewPort->DHeight = m_Height;
  ...
  for(int i = 0; i < 2; i++)
  {
    if(m_pBitMapArray[i] != NULL)
    {
      ...
      for (int iPlane = 0; iPlane < m_Depth; iPlane++)
      {
        if (m_pBitMapArray[i]->Planes[iPlane] != NULL)
        {
          FreeRaster(m_pBitMapArray[i]->Planes[iPlane],
                     m_Width, m_Height);
