...
bool ImgLoaderRawBitMap::Load(const char* pFileName)
{
  ...
  if(m_pBitMap == NULL)
  {
    ...
    m_pBitMap = AllocBitMap(m_Width, m_Height, m_Depth, flags, NULL);
  }
  ...
  ULONG flags = GetBitMapAttr(m_pBitMap, BMA_FLAGS);
  if (flags & BMF_INTERLEAVED)
  {
    ...
    if (Read(fileHandle, m_pBitMap->Planes[0], planeSize) != planeSize)
    {
      // Error while reading
      setErrorMsg(m_pLoadError, pFileName);
      FreeBitMap(m_pBitMap);
  ...
  else
  {
    ...
    {
      if (Read(fileHandle, m_pBitMap->Planes[i], planeSize) != planeSize)
      {
        // Error while reading
        setErrorMsg(m_pLoadError, pFileName);
        FreeBitMap(m_pBitMap);
